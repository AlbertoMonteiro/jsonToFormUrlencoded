...
test('Posso transformar um json com mais de uma propriedade simples em um form serializado', function () {
    ...
    equal(a.serializeJson(model), 'Nome=Alberto&Idade=21');
});
...
test('Posso transformar um json aninhando so com propriedades simples em um form serializado', function () {
    ...
    equal(a.serializeJson(model), 'Nome=Alberto&Idade=21&Endereco.Logradouro=Rua+Paulino+Nogueira&Endereco.Ap.Numero=283');
});
...
test('Posso transformar um json que contenha propriedades do tipo array de objeto simples em um form serializado', function () {
    ...
    equal(a.serializeJson(model), 'Nome=Alberto&Idade=21&Valores[0]=1&Valores[1]=2&Valores[2]=3');
});
...
test('Posso transformar um json que contenha propriedades do tipo array de objeto complexo em um form serializado', function () {
    ...
    equal(a.serializeJson(model), 'Nome=Alberto&Idade=21&Valores[0].Valor=1&Valores[1].Valor=2');
});
...
test('Posso transformar um json em um form serializado', function () {
    ...
    equal(a.serializeJson(model), 'Codigo=1&Nome=Fortes+Informatica&Estabelecimentos[0].Codigo=1&Estabelecimentos[0].Nome=Matriz&Estabelecimentos[1].Codigo=2&Estabelecimentos[1].Nome=Benfica');
});
